...
- **That reaper must not restart a run that is waiting on a resume.** `store.claim_suspended` flips `suspended → running`, so a run whose resume background task was lost to a restart is indistinguishable *by status* from a run whose worker crashed mid-execution — and `execute_run` deliberately accepts `running` for the latter. Re-invoking `execute_run` on the former would rebuild `messages` from the goal, overwrite `runs.transcript`, and duplicate the event log, destroying the only state a resume can continue from. `execute_run` now refuses any run whose transcript ends in unanswered `tool_use` blocks (`runtime._awaits_resume`); the reaper must route those to `resume_run`, not restart them.
